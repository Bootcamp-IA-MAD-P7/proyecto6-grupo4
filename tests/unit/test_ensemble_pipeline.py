...
from src.candidates.ensemble import build_ensemble_pipeline, build_ensemble_pipeline_abcd
...
def test_ensemble_abcd_fits_only_the_common_feature_schema_and_predicts_valid_classes() -> None:
...
def test_ensemble_abcd_is_a_soft_voting_combination_of_the_four_candidates() -> None:
    voting = build_ensemble_pipeline_abcd().named_steps["classifier"]
    assert voting.voting == "soft"
    estimator_names = {name for name, _ in voting.estimators}
    assert estimator_names == {"logistic_a", "hgb_b", "rf_c", "svc_d"}


def test_ensemble_abcd_exposes_predict_proba_over_the_three_classes() -> None:
    features, labels = _larger_sample()
    pipeline = build_ensemble_pipeline_abcd()
    pipeline.fit(features.loc[:, MODEL_FEATURES], labels)
    probabilities = pipeline.predict_proba(features.loc[:, MODEL_FEATURES])
